document.addEventListener('DOMContentLoaded', function () {
    ...
    async function criarTarefa() {
        ...
        try {
            ...
            const resposta = await fetch('https://focustime-2ha8.onrender.com/tarefa', {
                method: 'POST',
    ...
    async function concluirTarefa(e) {
        ...
        try {
            const res = await fetch(`https://focustime-2ha8.onrender.com/tarefa/${id}`);

            if (!res.ok) throw new Error("Tarefa não encontrada no servidor.");

            const tarefa = await res.json();

            const deleteRes = await fetch(`https://focustime-2ha8.onrender.com/tarefa/${id}`, {
                method: 'DELETE'
            });
